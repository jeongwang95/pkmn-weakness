...
// Gather damage relations for pokemon that only has a single type then convert data to a string
const dmgRelations = async (types) => {
    ...
    for (const k in dmgData) {
        if (k == 'double_damage_from' || k == 'half_damage_from' || k == 'no_damage_from') {
            s += `${k}: `;
            dmgData[k].forEach((type) => s += `${type.name} `);
            s += '<br>';
        }
    }
    return s;
...
const dmgRelations2 = async (types) => {
...
form.addEventListener('submit', async (event) => {
    ...
    if (data.types.length > 1) {
        dmgData = await dmgRelations2(data.types);
    } else {
        dmgData = await dmgRelations(data.types);
    }
